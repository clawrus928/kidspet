...
1. 在**原本的裝置**:家長設定 → ⚙️ 設定 → 雲端同步,會看到一組「家庭代碼」(例 `PET-X7K2M9`),點「複製」
2. 在**新裝置**:打開 App,首頁點「📲 輸入家庭代碼同步」,貼上同一組代碼 → 資料立刻同步過來

之後任一台裝置的操作(完成任務、餵寵物、核准…)都會即時反映到其他裝置。

> 安全性說明:採「家庭代碼即金鑰」的簡化設計,知道隨機代碼的人可讀寫該家庭資料,適合家庭自用。需要更嚴格的權限控管時可改接 Supabase Auth。

## 🌐 部署上線(讓手機能用)

這是純前端網站,可免費部署到 Vercel / Netlify / Cloudflare Pages:

- **Build 指令**:`npm run build`
- **輸出目錄**:`dist`
- **環境變數**:在平台的專案設定填入 `VITE_SUPABASE_URL` 與 `VITE_SUPABASE_ANON_KEY`(要啟用雲端同步時)

部署後得到一個網址,手機瀏覽器打開即可「加入主畫面」當 App 用。
